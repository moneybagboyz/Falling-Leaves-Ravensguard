...
- [ ] **P4-03** Implement combat scenario bootstrap: a debug scene (`CombatTestScene`) hard-spawns two formations of 10 combatants each on a flat extracted section of the sub-region grid with preset weapon/armor loadouts. Bypasses full game flow. Used to verify all combat tasks without requiring character creation or a live bandit camp. Gate behind a `--debug` launch flag.
- [ ] **P4-04** Define body zone data: head, neck, chest, abdomen, left/right arm, left/right leg — each with `hit_weight: float`, `wound_thresholds: {graze, wound, severe, lethal}`, and `critical_effects: []`. Store in `data/body_zones/*.json` validated by `body_zone.schema.json`.
- [ ] **P4-05** Define weapon data: `reach_class` (unarmed/short/medium/long/polearm), `damage_type` (slash/pierce/blunt), `swing_momentum`, `thrust_momentum`, `attack_speed`, `stamina_cost`, `min_skill`. Store in `data/weapons/*.json` validated by `weapon.schema.json`. Minimum starter set: dagger, short_sword, spear, axe, club.
- [ ] **P4-06** Define armor coverage data: per body zone `coverage: float` (0–1), `material` (gambeson/leather/mail/plate), `encumbrance`, `durability`. Store in `data/armor/*.json` validated by `armor.schema.json`. Minimum starter set: gambeson, leather_vest, mail_hauberk, iron_helm.
- [ ] **P4-07** Add `stamina: float` to `PersonState` (clamped 0–1). Stamina is a fast tactical resource distinct from `needs.fatigue` (slow strategic drain). Drains per attack by `weapon.stamina_cost`; recovers by `+0.15` per WEGO turn at rest. At stamina < 0.3: −20% accuracy and −1 reach effective class. At stamina 0: attacks always miss.
- [ ] **P4-08** Implement `CombatantState`: fields — `combatant_id: String`, `formation_id: String`, `team_id: String`, `tile_pos: Vector2i`, `z_level: int`, `body_zones: Dictionary` (zone_id → wound list), `equipment_refs: Array[String]`, `resolved_actions: Array`, `pain: float`, `bleeding: float`, `shock: float`, `stamina: float`. Derives from `PersonState` on combat entry via `CombatantState.from_person(p: PersonState)`.
- [ ] **P4-09** Implement `FormationState`: fields — `formation_id: String`, `team_id: String`, `member_ids: Array[String]` (CombatantState IDs), `anchor_pos: Vector2i`, `order: String` (advance/hold/charge/flank/retreat), `morale: float` (0–1). `morale` drops when members are killed or shocked; at morale < 0.2 the formation automatically issues a retreat order. `FormationState` does not have body zones or wounds — only members do.
- [ ] **P4-10** Implement basic inventory and equipment UI: player can pick up item entities from `x` chest tiles in `LocalView`, view carried items in a sidebar, and equip/unequip weapons and armor to `PersonState.equipment_refs`. Required so `CombatantState.equipment_refs` is populated before combat begins; without it all combatants fight unarmed/unarmored.
...
- [ ] **P4-19** Implement post-battle state resolution: write wounds back to surviving `PersonState` records; remove dead entities from `WorldState`; generate a loot pool from defeated combatants' `equipment_refs`; prompt player to recover items. Bandit camp building is removed from the world tile if all defenders are dead.
- [ ] **P4-20** Implement recovery: wounds heal over strategic time driven by rest (shelter quality), food (hunger below 0.3 halves healing rate), and care skill of any attending character. Severe wounds leave a permanent injury flag that caps the zone's mobility. Lethal wounds that were survived (shock incapacitation, not death) leave a scar with a minor permanent stat penalty.
- [ ] **P4-21** Write combat unit tests: hit location distribution across all body zones sums to 1.0; armor mitigation output matches analytic values for all material/damage-type combinations; WEGO resolution loop always terminates; stamina stays clamped in [0, 1]; morale retreat triggers correctly at threshold.
