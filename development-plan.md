...
**Time estimate:** 3–4 months  
**Exit gate:** A 20v20   fight (2 formations per side, 10 soldiers each) on a tactical map resolves with body-part injuries, armor mitigation, weapon effects, formation WEGO order influence, and a clear winner. No crashes, no degenerate loops.

#### Architecture — Formation-first design

Combat is designed to scale from 4v4 skirmishes to 1500v1500 pitched battles without a rewrite. The key abstraction is `FormationState`:

- **`CombatantState`** — one record per individual soldier. Tracks tile position, body zones, wounds (bleeding, pain, shock), stamina, equipment. Inspired by Dwarf Fortress body-part simulation. Always per-soldier, never aggregated.
- **`FormationState`** — one record per squad (10–50 soldiers in late game; 1 soldier in Phase 4 tests). Holds the list of `CombatantState` member IDs, the formation's tile anchor, its current WEGO order, and morale. The player issues orders at formation level; individual soldiers resolve within those orders.
- **WEGO resolution** — player selects formation from a sidebar list, assigns one order (advance, hold, charge, flank, retreat) from a dropdown, then confirms. All formations (friendly and enemy) resolve simultaneously tile-by-tile.
- **CombatView map** — uses the sub-region tile grid when fighting outdoors; uses LocalView tile grid when combat starts inside a building. Formations rendered as coloured blocks with a member-count label; clicking a formation drills down to per-soldier wound status.

#### Tasks

- [x] **P4-01** Implement `LocalView` scene: when the player interacts with a building cell in `SettlementView`, parse its `local_layout` (25×25 ASCII grid from `data/buildings/*.json`) and render it tile-by-tile. Tile key: `#` wall · `.` floor · `+` door · `~` water · `,` dirt · `_` open · `b` bed · `t` table · `c` counter · `s` shelf · `f` forge/fire · `^` stairs up · `v` stairs down · `%` crop · `*` rubble · `x` chest · `A` anvil · `M` millstone · `p` pillar. Player pawn moves tile-by-tile. `player_location.lx`/`ly` become live. `LocalView` is the default view when inside a building; `SettlementView` demoted to a map overlay toggled with Tab. Exiting any door tile returns the player to `SettlementView` at the parent cell. _(Standalone — can be completed independently of combat tasks.)_
- [x] **P4-02** Add `bandit_camp` building type to `BuildingPlacer`: placed outside settlement territory radius on tier 1+ tiles; stores `group_size` (4–12) and `gear_tier` (`ragged / light / medium`) in world tile data. `SubRegionGenerator` renders it as a cluster of rough shelters. Bandits are hostile to the player by default and visible in `SettlementView` as red pawns. This is the primary source of combat encounters for Phase 4.
- [x] **P4-03** Implement combat scenario bootstrap: a debug scene (`CombatTestScene`) hard-spawns two formations of 10 combatants each on a flat extracted section of the sub-region grid with preset weapon/armor loadouts. Bypasses full game flow. Used to verify all combat tasks without requiring character creation or a live bandit camp. Gate behind a `--debug` launch flag.
- [x] **P4-04** Define body zone data: head, neck, chest, abdomen, left/right arm, left/right leg — each with `hit_weight: float`, `wound_thresholds: {graze, wound, severe, lethal}`, and `critical_effects: []`. Store in `data/body_zones/*.json` validated by `body_zone.schema.json`.
- [x] **P4-05** Define weapon data: `reach_class` (unarmed/short/medium/long/polearm), `damage_type` (slash/pierce/blunt), `swing_momentum`, `thrust_momentum`, `attack_speed`, `stamina_cost`, `min_skill`. Store in `data/weapons/*.json` validated by `weapon.schema.json`. Minimum starter set: dagger, short_sword, spear, axe, club.
- [x] **P4-06** Define armor coverage data: per body zone `coverage: float` (0–1), `material` (gambeson/leather/mail/plate), `encumbrance`, `durability`. Store in `data/armor/*.json` validated by `armor.schema.json`. Minimum starter set: gambeson, leather_vest, mail_hauberk, helm.
- [x] **P4-07** Add `stamina: float` to `PersonState` (clamped 0–1). Stamina is a fast tactical resource distinct from `needs.fatigue` (slow strategic drain). Drains per attack by `weapon.stamina_cost`; recovers by `+0.15` per WEGO turn at rest. At stamina < 0.3: −20% accuracy and −1 reach effective class. At stamina 0: attacks always miss.
- [x] **P4-08** Implement `CombatantState`: fields — `combatant_id: String`, `formation_id: String`, `team_id: String`, `tile_pos: Vector2i`, `z_level: int`, `body_zones: Dictionary` (zone_id → wound list), `equipment_refs: Array[String]`, `resolved_actions: Array`, `pain: float`, `bleeding: float`, `shock: float`, `stamina: float`. Derives from `PersonState` on combat entry via `CombatantState.from_person(p: PersonState)`.
- [x] **P4-09** Implement `FormationState`: fields — `formation_id: String`, `team_id: String`, `member_ids: Array[String]` (CombatantState IDs), `anchor_pos: Vector2i`, `order: String` (advance/hold/charge/flank/retreat), `morale: float` (0–1). `morale` drops when members are killed or shocked; at morale < 0.2 the formation automatically issues a retreat order. `FormationState` does not have body zones or wounds — only members do.
- [x] **P4-10** Implement basic inventory and equipment UI: player can pick up item entities from `x` chest tiles in `LocalView`, view carried items in a sidebar, and equip/unequip weapons and armor to `PersonState.equipment_refs`. Required so `CombatantState.equipment_refs` is populated before combat begins; without it all combatants fight unarmed/unarmored.
- [x] **P4-11** Implement combat trigger: from `WorldView`, clicking a bandit camp tile reveals an "⚔ ATTACK CAMP" button; pressing it runs `_trigger_camp_combat` which converts the player and the bandit group into `FormationState` + `CombatantState` records, sets `WorldState.active_battle`, and transitions to `CombatView` via `SceneManager.push_scene`.
- [x] **P4-12** Implement WEGO order system — player side: during the planning phase, a sidebar lists all friendly formations. Player selects a formation, picks an order from a dropdown (advance / hold / charge / flank / retreat), optionally sets a target tile or enemy formation, then presses Confirm. All formation orders are locked simultaneously before resolution begins.
- [x] **P4-13** Implement WEGO order system — enemy AI: each enemy formation picks an order using simple rules — advance toward nearest enemy formation if out of melee reach; charge if adjacent; retreat if morale < 0.2. AI resolves all enemy orders in one pass before the simultaneous resolution step. Must not loop or stall.
- [x] **P4-14** Implement WEGO resolution: all formations execute their orders simultaneously, tick by tick. Each soldier within a formation steps toward the formation anchor's target; on reaching melee range of an enemy soldier, attack resolution triggers. Resolve all moves first, then all attacks, to avoid order-dependence.
- [x] **P4-15** Implement attack resolution: attacker adjacent to target → roll hit location weighted by `body_zone.hit_weight` → compare weapon momentum type vs. zone armor `coverage` and `material` → derive wound severity → append wound to `CombatantState.body_zones[zone]`. Severity levels: `none / graze / wound / severe / lethal`.
- [x] **P4-16** Implement wound effects: each wound on a zone adds to `bleeding` (rate per turn), `pain` (instantaneous spike), and a mobility penalty. `shock = pain + bleeding_accumulated`; at shock ≥ 1.0 the combatant is incapacitated. At lethal wound on chest/head, combatant dies immediately. Multiple wounds across zones stack all three floats.
- [x] **P4-18** Implement `CombatView`: full-screen tactical renderer using either sub-region tiles (outdoor) or LocalView tiles (indoor) as the map. Formations rendered as coloured blocks with member count. Turn-phase indicator (Planning / Resolving / Results). Click formation to drill down to per-soldier wound summary panel. Order dropdown and Confirm button in sidebar.
- [x] **P4-19** Implement post-battle state resolution: write wounds back to surviving `PersonState` records; remove dead entities from `WorldState`; generate a loot pool from defeated combatants' `equipment_refs`; prompt player to recover items. Bandit camp building is removed from the world tile if all defenders are dead.
- [x] **P4-20** Implement recovery: wounds heal over strategic time driven by rest (shelter quality), food (hunger below 0.3 halves healing rate), and care skill of any attending character. Severe wounds leave a permanent injury flag that caps the zone's mobility. Lethal wounds that were survived (shock incapacitation, not death) leave a scar with a minor permanent stat penalty.
- [x] **P4-21** Write combat unit tests: hit location distribution across all body zones sums to 1.0; armor mitigation output matches analytic values for all material/damage-type combinations; WEGO resolution loop always terminates; stamina stays clamped in [0, 1]; morale retreat triggers correctly at threshold.
...
**Time estimate:** 3–4 months  
**Exit gate:** A character owns a workshop (purchased or built), has at least one follower, has bought and sold goods on the open market, and can demonstrate compounding profit over 30 in-game days — all without crashes or save-load regressions.

#### Pre-Phase 5 infrastructure gaps

These must exist before Phase 5 code can run cleanly:

- **`PersonState.ownership_refs`** exists but is never populated. `property_core` (P5-01) will be the first system to write it.
- **`PersonState.reputation`** exists as a `Dictionary` but nothing writes to it. Phase 5 adds the first reputation events (P5-09).
- **No follower tracking field on `PersonState`**. A `follower_ids: Array[String]` field must be added to track which NPCs serve the player; combat trigger must iterate it when building formations.
- **`SettlementPulse` routes all production output to `SettlementState.inventory`**. With player-owned buildings, a post-pulse income hook (wired into `property_core`) must redirect the owner's share before the next consumption tick.
- **`BuildingPlacer` only runs at worldgen**. Player-initiated construction (P5-10) needs a `ConstructionJob` queue on `WorldState` and a `ConstructionSystem` tick hook in phase `production_pulse` — written as an entirely separate code path from `BuildingPlacer`.

#### Tasks

- [x] **P5-01** Implement `property_core`: tracks ownership of buildings and land by entity ID (`PersonState.ownership_refs`); assigns income rights (fraction of production output credited to owner each pulse) and upkeep obligations (coin drained from owner each pulse). Integrates with `SettlementPulse` via a post-pulse income hook so the owner's share is redirected before consumption. Persists ownership in `WorldState.property_ledger: Dictionary` (building_instance_id → owner_person_id).
- [x] **P5-02** Implement workshop ownership — two paths:
  - **Purchase**: player interacts with an existing production building (smithy, grain_mill, etc.) in `SettlementView`; if the building is unowned and the player has enough coin (based on `construction_cost.coin` in the building's JSON), the transaction completes and `property_core` records the transfer.
  - **Build** (see P5-10 for the construction system): player selects an `open_land` cell in a settlement territory, picks a building type from a filtered list (category = `production`), is shown the full `construction_cost`, and confirms. This starts a `ConstructionJob`; on completion the building is usable and owned by the player.
  - In both cases: player can then assign NPC workers to fill the building's labor slots via the dialogue system; worker output routes through `property_core`'s income hook.
- [x] **P5-03** Implement player trade: buy goods from `SettlementState.market_inventory` (coin transferred, inventory decremented); sell goods from personal inventory into settlement market (coin received, inventory incremented, price responds to volume). Trading skill adjusts effective buy/sell prices (each level = 0.5% margin improvement). Price impact is applied to `SettlementState.prices` after each transaction.
- [x] **P5-04** Implement camp founding: player can spend resources to establish a camp on any `open_land` tile outside all settlement territory radii. Camp is represented as a `SettlementState` with `tier = 0` and `is_player_camp: bool = true`, registered in `WorldState.settlements`. The pulse for player camps skips trade party spawning and population growth; all production surplus credits directly to the player via `property_core`. Camp renders in `SubRegionGenerator` as a cluster of rough shelters (reuse `bandit_camp` visual treatment). Followers assigned to the camp occupy its labor slots.
- [x] **P5-05** Implement follower system: add `follower_ids: Array[String]` to `PersonState`; followers are full `PersonState` records with `active_role` set to their assigned duty (`guard`, `laborer`, `assistant`). Followers consume wages (deducted from player coin each strategic tick) and food (deducted from player or camp inventory). In labor roles they contribute labor to the building or camp they are assigned to. In combat, followers are added to the player's `FormationState` as full `CombatantState` members when a battle is triggered — `_trigger_camp_combat` and the combat bootstrap path must iterate `player.follower_ids` and include them.
- [x] **P5-06** Implement basic recruitment: player can approach any NPC whose `schedule_state = "wandering"` or `active_role = ""` (unemployed) and make a wage offer. Acceptance requires: (a) offered wage ≥ NPC's minimum (derived from population_class), AND (b) player reputation ≥ 0.0 with the NPC's associated faction (or NPC has no faction tie). On acceptance, the NPC's `person_id` is added to `PersonState.follower_ids`; their `active_role` is set; their slot is removed from `SettlementState.labor_slots` if they held one. Reputation needed here means P5-09 must be complete first.
- [x] **P5-07** Implement small armed group management: a `GroupState` class wraps the player's follower_ids with additional fields: `pay_ledger: float` (coin owed per tick), `morale: float` (0–1, decays when unpaid, recovers when paid and fed). UI shows size, equipment summary, pay rate. At `morale < 0.2` followers begin deserting (removed from follower_ids one per tick until paid). `GroupState` serialises into `WorldState` alongside the player's `PersonState`.
- [x] **P5-08** Implement `OwnershipView` UI: panel accessible from the main HUD showing all player-owned assets (buildings with income/upkeep per tick), current follower roster with role and morale, and a personal ledger graph of coin over the last 30 ticks.
- [x] **P5-09** Implement reputation system: `PersonState.reputation: Dictionary` (faction_id → float, range −1.0 to +1.0) is already defined but never written. Add `ReputationEvents` — a static module that posts delta events to `EventQueue` and a handler that applies them at end of tick. Events that grant positive reputation: defeating a faction's enemy, completing a trade deal through a faction settlement, completing a work contract. Events that reduce reputation: crimes in faction territory, killing faction-aligned NPCs, defaulting on wages. Reputation is displayed in the player's `OwnershipView` and gates recruitment (P5-06) and future faction interactions.
- [x] **P5-10** Implement building construction system: `ConstructionJob` data class (`job_id`, `building_id`, `cell_id`, `owner_id`, `ticks_remaining`, `resources_committed: Dictionary`). `WorldState.construction_jobs: Dictionary` stores active jobs. `ConstructionSystem` runs each `production_pulse` tick: decrements `ticks_remaining` (derived from `construction_cost.labor_days`); on completion stamps `building_id` onto the cell via the same path as `BuildingPlacer`, creates labor slots on the owning settlement, and records ownership in `property_core`. Resources are deducted from the player's personal inventory (or camp stock) when the job begins — if resources are insufficient, the job is queued but not started.
