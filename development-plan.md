...
- [ ] **P4-11** Implement combat trigger: from `SettlementView`, when the player moves adjacent to a bandit camp cell, an "Attack" option appears. Pressing it converts the player's party and the bandit group into `FormationState` + `CombatantState` records and transitions to `CombatView`. Bandits also trigger an attack on the player if within 3 cells and the player does not retreat.
- [ ] **P4-12** Implement WEGO order system — player side: during the planning phase, a sidebar lists all friendly formations. Player selects a formation, picks an order from a dropdown (advance / hold / charge / flank / retreat), optionally sets a target tile or enemy formation, then presses Confirm. All formation orders are locked simultaneously before resolution begins.
...
- [ ] **P4-17** Implement terrain effects on the combat map: elevation difference of ≥1 z-level grants +15% hit chance to higher combatant; obstacle tiles (walls, dense trees) block melee reach and line-of-sight; difficult ground (mud, shallow water) costs +1 movement per tile.
- [ ] **P4-18** Implement `CombatView`: full-screen tactical renderer using either sub-region tiles (outdoor) or LocalView tiles (indoor) as the map. Formations rendered as coloured blocks with member count. Turn-phase indicator (Planning / Resolving / Results). Click formation to drill down to per-soldier wound summary panel. Order dropdown and Confirm button in sidebar.
